...
router.post('/cancel-subscription', async (req, res) => {
    const { email, tipoSubscricao } = req.body;
    ...
    try {
      ...
      // Encontrar as assinaturas ativas do cliente
      const subscriptions = await stripe.subscriptions.list({
        customer: customer.id,
        status: 'active'
      });
  
      // Filtrar a assinatura específica pelo tipo
      const subscription = subscriptions.data.find(sub => sub.metadata.tipoSubscricao === tipoSubscricao);
      ...
      const query = "UPDATE atleta SET estado = 'cancelled', data_expiracao_mensalidade = $2 WHERE user_id = $1";
